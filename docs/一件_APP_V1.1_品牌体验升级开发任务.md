...
> 规划日期：2026-09-05  
> 任务状态：开发完成（含 Xiaomi Android 14 实体机与正式覆盖安装验收）
> 建议版本：`1.1.0 (2)`  
...
- [x] 将本次权威 JPG、动效规范和本任务计划纳入新分支首个提交；
- [x] 确认 V1 RC 标签仍指向 `65314617803b4a8b95b846963a8389e6afffdd53`；
- [x] 执行当前 JVM、Lint、Debug、AndroidTest 和 Release 构建，记录新任务开始前基线；
- [x] 确认手机正式版数据不用于破坏性测试。

### WP1：目标图标资源与领域模型

- [x] 从 `design/Android Adaptive Icon/goal-icons/android-vector/` 复制 8 个正式 Vector Drawable 到 App；
- [x] 建立稳定的 `GoalIconKey` 枚举或值对象，数据库只保存稳定字符串 key，不保存资源 ID；
- [x] 为未知 key 提供 `other` 回退；
- [x] 建立 key、Drawable、中文名称与无障碍描述的单一映射；
- [x] 校验所有图标在黄/绿容器、深浅背景、1.0×/2.0×字体和 600dp+ 布局中的清晰度。

### WP2：Room v1→v2 非破坏迁移

- [x] `GoalEntity` 增加 `iconKey TEXT NOT NULL DEFAULT 'other'`；
- [x] Room 数据库版本从 1 升到 2；
- [x] 提供显式 `Migration(1, 2)`，禁止 destructive migration；
- [x] 更新 `app/schemas/.../2.json`；
- [x] 现有活动目标和历史目标迁移后统一为 `other`，不按名称猜测；
- [x] 迁移后 Goal、Completion、提醒设置和当前完成状态必须完整保留。

### WP3：目标图标选择与展示

- [x] 创建目标页增加 8 个图标的可横向换行选择器；
- [x] 每项至少 48dp 热区，并同时提供图形、选中状态和可读标签；
- [x] 默认 `other`，开始坚持时和目标名称一起原子保存；
- [x] 设置页支持修改当前目标图标；
- [x] 首页、记录页和历史卡片显示持久化后的图标；
- [x] 不增加底部导航，不扩大为多目标管理或图标商店。

### WP4：Android 系统启动页

- [x] 在版本目录中锁定 `androidx.core:core-splashscreen`；
- [x] 新建 `Theme.OneThing.Starting`，配置暖白背景、A1 Logo 和 `postSplashScreenTheme`；
- [x] Manifest 只给 `MainActivity` 使用 Starting Theme；
- [x] 在 `MainActivity.onCreate()` 调用 `installSplashScreen()`，位置在 `super.onCreate()` 之前；
- [x] 使用轻量退出动画，不人为延长冷启动；
- [x] 验证 API 26、API 31+、API 36 的冷启动、热启动、深色系统主题和旋转；
- [x] 验证通知点击、进程恢复和首次欢迎页路由不受影响；
- [x] 输出 `design/screens/00 启动页.png` 和对应验收截图。

### WP5：Logo 点亮完成动画资源

- [x] 将 A1 Logo 拆成 `Number_1` 与 `Status_Point`；
- [x] 按权威稿制作 `Glow`、`Orbit_Path`、`Particle`、`Success_Check`；
- [x] 基础动画严格按 `0–1800ms` 七阶段时间轴制作；
- [x] 黄点缩放使用 `1 → 1.25 → 1.05`；
- [x] 半环路径从右下向右、向上运动，不闭合、不做完整 360°；
- [x] 粒子为 3–8 个小圆点或光点，不使用彩带；
- [x] 黄点均匀过渡到 `#59AD5E`，完成后光晕缓慢消散；
- [x] 校验 JSON 可解析、无外部图片依赖、无文字图层、单个文件建议不超过 200KB。

### WP6：Compose 接入与状态控制

- [x] 保持“Completion 写入成功后才显示完成动画”的现有业务顺序；
- [x] 点击后立即防重入，按钮视觉压至 96% 并轻回弹；
- [x] 使用可控 progress 接入 7 阶段，动态显示今天完成、连续第 N 天和节点文案；
- [x] 动画期间返回、切后台或进程重建时直接进入已完成首页，不重复播放；
- [x] 动画加载失败时使用现有 Compose Overlay；
- [x] 系统动画比例为 0 或启用“移除动画”时，跳过轨迹/粒子并快速显示完成结果；
- [x] TalkBack 只朗读一次完整完成信息，装饰轨迹和粒子不进入语义树。

### WP7：测试、真机与文档

- [x] 单元测试覆盖 icon key、未知值回退、连续天数和完成状态；
- [x] Room migration test 从真实 v1 schema 升到 v2，并验证数据完整；
- [x] Compose UI 测试覆盖图标选择、默认值、设置修改、重启持久化和历史展示；
- [x] 完成动画测试覆盖单次触发、数据先写、正常结束、中断、加载失败和移除动画；
- [x] 启动页测试覆盖首次、已有目标、通知打开、冷/热启动和无双闪屏；
- [x] API 26、API 36 和 Xiaomi Android 14 执行回归；API 26/36 全量设备测试各 46/46，实体机无界面自动化 17/17 并完成独立 Debug 人工主流程；
- [x] Release 使用同一正式签名覆盖安装，确认 v1→v2 数据迁移、图标默认值和提醒状态保留；实体机 `1.0.0 → 1.1.0` 未卸载、未清数据覆盖通过；
- [x] 更新 PRD、决策基线、开发进度、验收方案和本地发布记录；
- [x] 记录最终 commit、APK SHA-256、签名验证和真机结果；最终代码提交为 `295b1dc`。
...
- [x] 8 个图标均可选择，默认与未知值稳定回退到 `other`；
- [x] 创建、设置、首页、记录和历史显示一致；
- [x] 数据库迁移、关闭重开和进程重建后选择不丢失；实体机正式覆盖后原目标、三天 Completion、提醒与默认图标均保留；
- [x] TalkBack 可读，选中状态不只依赖颜色。

### 6.2 启动页

- [x] 无黑屏、白闪、双 Splash 或人为长时间停留；
- [x] A1 Logo 安全区正确，背景和状态栏颜色连续；
- [x] API 26、Android 12+ 和 API 36 行为一致；
- [x] 首次、已有目标和通知点击均进入正确页面。

### 6.3 完成动画

- [x] 视觉与权威 JPG 的 7 阶段顺序一致；
- [x] 总时长在 1.5–1.8 秒，动画不循环；
- [x] 数据保存成功后才播放，点击不会重复写入；
- [x] 动画中断和进程恢复后直接显示已完成状态；
- [x] 动态连续天数正确，Lottie 内无写死文字；
- [x] 动画关闭或资源失败时有可用降级路径；
- [x] 旧版底部导航和旧页面壳层未被带回。

### 6.4 工程与发布

- [x] JVM、Room、Compose UI、API 26/API 36 设备测试和 Lint 全部通过；
- [x] Debug、AndroidTest、Release 构建成功；
- [x] Release 仍无 `INTERNET`、第三方统计和无关敏感权限；
- [x] Lottie 单文件建议不超过 200KB，新增依赖后的 APK 体积变化有记录；
- [x] 正式签名实体机覆盖安装成功，原目标、完成记录和提醒设置不丢失；
- [x] 文档、设计资产、代码、测试和发布记录与 Git commit 对应。
