...
- [ ] API 26、API 36 和 Xiaomi Android 14 Debug 包执行完整回归；API 26/36 已完成，Xiaomi 实体机按用户要求暂缓；
- [ ] Release 使用同一正式签名覆盖安装，确认 v1→v2 数据迁移、图标默认值和提醒状态保留；签名与自动迁移已验证，实体机覆盖按用户要求暂缓；
- [x] 更新 PRD、决策基线、开发进度、验收方案和本地发布记录；
- [x] 记录最终 commit、APK SHA-256、签名验证和真机结果；真机结果明确记录为暂缓。
...
- [x] 数据库迁移、关闭重开和进程重建后选择不丢失；实体机覆盖安装暂缓；
...
- [ ] 正式签名实体机覆盖安装成功，原目标、完成记录和提醒设置不丢失；按用户要求暂缓；
