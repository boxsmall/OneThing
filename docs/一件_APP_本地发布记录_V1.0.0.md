...
> 最后更新：2026-09-04
> 发布渠道：本地签名 APK
> 当前状态：签名构建、静态门禁、真机核心闭环与 HyperOS 锁屏自然提醒通过；Release Candidate 分支和标签已同步 GitHub；签名材料已生成加密本地备份并完成恢复校验，待转移到独立离线介质
...
| Release Candidate 提交 | `65314617803b4a8b95b846963a8389e6afffdd53` |
| 归档分支 | `codex/v1.0.0-rc1-archive` |
| 归档标签 | `v1.0.0-rc.1`；固定指向 Release Candidate 提交，不因后续文档补记而移动 |
| GitHub 同步 | 归档分支与注释标签均已推送至 `https://github.com/boxsmall/OneThing`；远端剥离标签与 Release Candidate 提交一致 |
...
- 2026-09-04 已将 `onething-release.jks`、`keystore.properties` 和备份清单写入 WinZip AES-256 加密压缩包。
- 压缩包：`D:\OneThing_Backups\20260904\OneThing-signing-backup-20260904.zip`；SHA-256：`FA44F49761F8603B8DAFEE9FE425DCA4A1FE2E380F8F4F24195799178E7A4116`。
- 恢复码位于同目录独立文本文件，具体内容不进入本文、聊天或 Git；SHA-256 清单与压缩包并列保存。
- 已使用恢复码实际解密压缩包，并逐项比较两个源文件的 SHA-256，结果一致；三个备份文件均位于 Git 仓库外，工作区保持干净。
- 当前原始签名材料和加密备份仍位于同一台电脑、同一块磁盘，因此只属于可恢复的本地副本，不等同于离线或异地备份。以后取得 U 盘、移动硬盘或另一台可信设备后，必须复制压缩包，并将恢复码分开保存。
...
- Git 归档以 `main@75df9c1` 为父提交，在独立分支 `codex/v1.0.0-rc1-archive` 保存，并以 `v1.0.0-rc.1` 标识；分支和标签已上传 GitHub，不改写 `main` 历史。归档范围包含 Android 工程、测试、设计源资源和全部同步文档，不包含构建缓存、APK、用户数据或签名材料。
...
- [x] 归档与产物完全对应的 Git commit，使用 `v1.0.0-rc.1` 标签建立可追溯引用，并将归档分支和标签同步到 GitHub。
- [x] 生成包含固定签名密钥与配置的 AES-256 加密本地备份，完成解密恢复和逐文件哈希校验；恢复码未写入 Git 或本文。
